...
def handler(message: dict, text: str, user: UserClass, message_id: int):
    ...
    match text:
        case 'Add Meme':
            ...
            user.send_message(user.translate('meme_type'), keyboards.en_meme_type)
        case 'Meme Count':
    ...
    if not_matched and user.database.rank in BOT_ADMINS:
        match text:
            ...
            case 'Delete Meme':
                ...
                user.send_message(user.translate('meme'), keyboards.en_back)
            case 'Ban Vote':
                user.database.menu = User.Menu.ADMIN_BAN_VOTE
                user.send_message(user.translate('meme'), keyboards.en_back)
            case 'Accept Meme':
                user.database.menu = User.Menu.ADMIN_ACCEPT_MEME
                user.send_message(user.translate('meme'), keyboards.en_back)
            case 'Deny Meme':
                user.database.menu = User.Menu.ADMIN_DENY_MEME
                user.send_message(user.translate('meme'), keyboards.en_back)
            case 'Edit Meme':
                user.database.menu = User.Menu.ADMIN_SEND_EDIT_MEME
                user.send_message(
                    user.translate('send_edit_meme'), keyboards.en_back
                )
            case 'File ID':
                user.database.menu = User.Menu.ADMIN_FILE_ID
                user.send_message(user.translate('send_document'), keyboards.en_back)
